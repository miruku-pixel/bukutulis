...
const SalesChart = () => (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-6 shadow-sm">
        <div className="flex items-center justify-between mb-8">
            ...
            <div className="px-3 py-1 bg-zinc-800 rounded-lg text-xs font-medium text-zinc-400 border border-zinc-700">Monthly</div>
        </div>
...
const ProductRankChart = () => (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-6 shadow-sm">
        <div className="flex items-center justify-between mb-8">
            ...
            <div className="px-3 py-1 bg-zinc-800 rounded-lg text-xs font-medium text-zinc-400 border border-zinc-700">Rank</div>
        </div>
                ...
                >
                    ...
                    <Bar dataKey="sales" radius={[0, 4, 4, 0]}>
                        {productRankData.map((entry, index) => (
                            <Cell key={`cell-${index}`} fill="#3b82f6" fillOpacity={1 - (index * 0.07)} />
                        ))}
                    </Bar>
                </BarChart>
            </ResponsiveContainer>
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col gap-2">
                ...
                <p className="text-zinc-400 text-sm">Welcome back! Here&apos;s what&apos;s happening with your projects today.</p>
            </div>

            {/* Stats Grid */}
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                {stats.map((stat, index) => (
                    <div key={index} className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-6 shadow-sm hover:border-zinc-700 transition-colors">
